...
const renderSong = (playlistName, song) => {
  return `
    <div class="song">
      ...
      <button class="remove-song" data-playlist="${playlistName}" data-id="${
    song.title
  }">Remove</button>
    </div>
  `;
  ...
  return `
    <section>
      ...
      <div class="sort-songs">
        <button class="criterion" data-id="${
          playlist.name
        }" data-criterion="duration">Sort by duration</button>
        <button class="criterion" data-id="${
          playlist.name
        }" data-criterion="title">Sort by title</button>
        <button class="criterion" data-id="${
          playlist.name
        }" data-criterion="artist">Sort by artist</button>
      </div>
      ${playlist.songs.map((song) => renderSong(playlist.name, song)).join("")}
      <button class="remove-playlist" data-id="${
        playlist.name
...
const initEventListeners = () => {
  ...
  playlistsElement.addEventListener("click", (event) => {
    if (event.target.matches("button.remove-playlist")) {
      // removePlaylist listener
      ...
      const updatedPlaylists = musicCatalog.getAllPlaylists();
      renderPlaylists(updatedPlaylists);
    } else if (event.target.matches("button.favorite-song")) {
      // favoriteSong listener
      const playlistId = event.target.dataset.playlist;
      const songId = event.target.dataset.id;
